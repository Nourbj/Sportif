...
const AdminMediaInput = ({
  ...
  const icon = isVideo ? '▶' : '📷';
  const uploadText = isVideo ? 'تحميل فيديو' : 'تحميل صورة';
  const linkLabel = isVideo ? 'أو رابط الفيديو' : 'أو رابط الصورة';
  ...
  return (
    <div>
      <label className="admin-media-label">{label} (تحميل ملف)</label>
      <label className="admin-image-upload-box">
        <input
          type="file"
          className="admin-image-file-input"
          onChange={e => { if (setFile) setFile(e.target.files[0]); if (onValueChange) onValueChange(''); }}
          accept={accept}
        />
      ...
      <input
        ...
        value={value}
        onChange={e => { if (onValueChange) onValueChange(e.target.value); if (setFile) setFile(null); }}
